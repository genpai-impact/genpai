...
namespace GalForUnity.System{
    public class GfuObjectPool : MonoBehaviour{
        ...
        public GameObject Get(UnityEngine.Transform otherTransform,int idx,bool rotation=true,bool scale=true){
            if (readyGameObjects.Count == 0){
                var instantiate = GameObject.Instantiate(obj[idx], otherTransform, true);
