### bv0.3.0.0_20220730_Alpha

1. 更改readme.md。
2. 重构VERSION.md（本文档）。

下图是新旧版本的对比：核心思想是引入主程进行流程控制。
![重构分支和主分支架构对比](重构和原来主要差别.jpg)
下图以攻击流程为例（先看Step会清晰很多），描述架构怎样运行。
![攻击流程](以攻击为例是怎样运作的.jpg)

# 分支版本号命名规则
## 版本号格式：bvx.y.z.n_mmmm_qqqqqq
###bv
代表branch-version
###x
代表版本更新，比如新增玩法，假期活动，每个版本上线一次加1，因此项目未上线前均为0。
###y
代表项目天翻地覆的发生了变化，导致接口不兼容，或者整个架构有了极大的变更。因此本重构分支前缀 起始为 bv0.3.z.n_mmmm_qqqqqq
###z
代表项目新增了比较大的模块功能。例如新增了天气系统、新增了库存管理、新增商店、新增选关模块等。

###n
代表进行了一些较大的bug修正，或者新增了一些小的功能。例如增加了一些卡牌、新增了一个buff、新增了一个关卡等。或代表进行了一些小的bug修整，或者进行了微小的改动。例如修改了某张卡牌的名字。
###m
代表时间，例如20220207
###q
代表版本阶段。
